...
# Note: you may wish to reduce the section length to avoid tokens being discarded with the model size.
# The default "all-MiniLM-L6-v2" is 256 tokens at runtime, which equates to about 3x the characters (768 characters)
MAX_SECTION_LENGTH = 1000
SENTENCE_SEARCH_LIMIT = 100
SECTION_OVERLAP = 100
PINECONE_ENCODING_MODEL = "all-MiniLM-L6-v2" 


parser = argparse.ArgumentParser(
    description="Prepare documents by extracting content from PDFs, splitting content into sections, uploading to blob storage, and indexing in a search index.",
    epilog="Example: prepdocs.py '..\data\*' --storageaccount myaccount --container mycontainer  --index myindex -v"
    )
...
parser.add_argument("--pineconeenv", help="Pinecone environment.")
...
# NOTE ABOUT FORMRECOGNIZERSERVICE:  It appears if you're using a free version, it will only return 2 pages.  Therefore, you may want to set the "localpdfparser" to True.
...
#Pinecone Index
pinecone.init(
        api_key=args.pineconekey,
        environment=args.pineconeenv
    )
pinecone_index = pinecone.Index(args.index)

#Default Encoder, GPU if available
...
def index_sections(filename, sections):
    ...
    vectors = []
    i = 0
    for id, vector, metadata in sections:
        vectors.append((id, vector, metadata))
        print(f"Section: '{metadata['content']}'")
        if i % 700 == 0:
           i += 1
